...
class Transaction:
    ...
    def initialize(self, email: str, amount: str, **kwargs) -> dict:
        ...
        required = {'email': email, 'amount':amount}
        # merge with other keywords meant for request body
        body = {**required, **kwargs}
    ...
    def verify(self, reference: str) -> bool:
        ...
        response = response.get('data').get('status')
    ...
    def fetch(self, id: int) -> dict:
        ...
        response = requests.get(
            url = "https://api.paystack.co/transaction/{}".format(id),
            headers = {
                'Authorization': self.auth
            }
        ).json().get('data')
